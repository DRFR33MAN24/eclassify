...
class ItemRepository {
  ...
  Future<Map> changeMyItemStatus(
    ...
    Map response = await Api.post(url: Api.updateServiceStatusApi, parameter: {
      Api.status: status,
      Api.itemId: itemId,
      Api.soldTo: userId
    });
  ...
  Future<Map> createFeaturedAds({required int itemId}) async {
    Map response = await Api.post(url: Api.makeServiceFeaturedApi, parameter: {
      "item_id": itemId,
    });
    return response;
  }
