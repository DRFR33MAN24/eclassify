...
import '../../../../data/cubits/category/services/fetch_service_sub_categories_cubit.dart';
  ...
  void pageScrollListen() {
    if (controller.isEndReached()) {
      if (context.read<FetchServiceCategoryCubit>().hasMoreData()) {
        context.read<FetchServiceCategoryCubit>().fetchCategoriesMore();
      }
    }
  }
                              ...
                              if (category.children!.isEmpty &&
                                ...
                                if (TouchManager.canProcessTouch()) {
                                  ...
                                  Navigator.pushNamed(context,
                                      Routes.selectServiceNestedCategoryScreen,
                                      arguments: {
                                        "current": category,
                                      });
                                  Future.delayed(Duration(seconds: 1), () {
...
class _SelectNestedCategoryState extends CloudState<SelectNestedCategory> {
  ...
  void getSubCategories() {
    if (widget.current.children!.isEmpty) {
      context
          .read<FetchServiceSubCategoriesCubit>()
  ...
  void pageScrollListen() {
    if (controller.isEndReached()) {
      if (context.read<FetchServiceSubCategoriesCubit>().hasMoreData()) {
        context
            .read<FetchServiceSubCategoriesCubit>()
            .fetchSubCategories(categoryId: widget.current.id!);
      }
  ...
  Widget fetchSubCategoriesData() {
    return BlocBuilder<FetchServiceSubCategoriesCubit, FetchServiceSubCategoriesState>(
      builder: (context, state) {
        if (state is FetchServiceSubCategoriesInProgress) {
          return shimmerEffect();
        }

        if (state is FetchServiceSubCategoriesFailure) {
